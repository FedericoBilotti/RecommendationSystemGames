...
public class RatingController(IRatingUseCase ratingUseCase) : ControllerBase
{
    ...
    public async Task<IActionResult> RateGame([FromRoute] Guid gameId, [FromBody] GameRateRequestDto gameRateRequestDto, CancellationToken cancellationToken)
    {
        ...
        bool result = await ratingUseCase.RateGameAsync(gameId, gameRateRequestDto.Rating, userId!.Value, cancellationToken);
    ...
    public async Task<IActionResult> DeleteRating([FromRoute] Guid gameId, CancellationToken cancellationToken)
    {
        ...
        bool result = await ratingUseCase.DeleteRatingAsync(gameId, userId!.Value, cancellationToken);
        return result ? Ok() : NotFound();
    }
